...
% Even though there is currently no comprehensive framework for generating proofs during the execution of CAD, several related
% methods already support this capability. One important remark is that all of these related methods either are incomplete,
% solve a more restricted version of the problem or are less efficient than CAD.
% Additionally, some approaches have been fully verified, in the sense that there has
% been a formal proof of their correctness within a proof assistant. This is another method for increasing the confidence one has
% in the results of such procedures. It has the advantage of eliminating the overhead that proof production imposes on the
% algorithm, but it also freezes the implementation of the algorithm, since a new proof would have to be written if the
